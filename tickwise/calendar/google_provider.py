...
Pushes Tickwise sessions as events on a Google Calendar via the v3
    ...
    def push_sessions(self, sessions: list[dict[str, Any]]) -> SyncReport:
        ...
        with httpx.Client(timeout=10.0) as client:
            for session in sessions:
                event_id = f"tickwisesession{session['id']}"
                payload = {
                    "id": event_id,
...
def _ensure_iso(value: str) -> str:
    """Google requires RFC 3339; Tickwise stores ISO-8601 already, but
