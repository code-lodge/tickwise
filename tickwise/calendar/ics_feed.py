...
def _build_with_icalendar(sessions: list[dict[str, Any]], include_descriptions: bool, calendar_name: str) -> str:
    ...
    cal = Calendar()
    cal.add("prodid", "-//Tickwise//tickwise//EN")
    cal.add("version", "2.0")
    cal.add("x-wr-calname", calendar_name)
    cal.add("method", "PUBLISH")
    for row in sessions:
        event = Event()
        event.add("uid", f"tickwise-session-{row['id']}@tickwise")
...
def _build_manual(sessions: list[dict[str, Any]], include_descriptions: bool, calendar_name: str) -> str:
    ...
    for row in sessions:
        ...
        lines.append(f"UID:tickwise-session-{row['id']}@tickwise")
