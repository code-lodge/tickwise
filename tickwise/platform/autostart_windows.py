...
Writes `HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\\Tickwise`
with the launch command so Tickwise starts automatically when the user
...
def is_enabled() -> bool:
    """True iff a `Tickwise` value exists under the Run key."""
    try:
        with winreg.OpenKey(  # type: ignore[attr-defined,unused-ignore]
