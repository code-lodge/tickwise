...
- `enable(command)` — register Tickwise to launch at login.
...
`python -m tickwise` invocation that mirrors how the user starts the
process today.
"""
...
def default_launch_command() -> str:
    """Return the canonical `python -m tickwise` command for the current Python."""
    return f"{shlex.quote(sys.executable)} -m tickwise"


if sys.platform == "win32":
    from tickwise.platform.autostart_windows import disable, enable, is_enabled
elif sys.platform == "darwin":
    from tickwise.platform.autostart_macos import disable, enable, is_enabled
elif sys.platform.startswith("linux"):
    from tickwise.platform.autostart_linux import disable, enable, is_enabled
